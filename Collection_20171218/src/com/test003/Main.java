...
public class Main {
	...
	public static void main(String[] args) {
		...
		//- TreeSet, TreeMap
		//- 검색 기능에 적합한 컬렉션
		//- 이진 트리는 여러 개의 노드가 트리 형태로 연결된 구조로, 루트 노드라고 불리는 하나의 노드에서부터 시작해서
		//  각 노드에 최대 2개의 노드를 연결할 수 있는 구조
		//- 이진 트리 구조는 연결된 두 노드를 부모-자식 관계에 있다고 한다.
		//- 이진 트리가 범위 검색을 쉬게 할 수 있는 이유는 값들이 정렬되어 있어 그룹핑이 쉽기 때문이다.
		
		//크기 비교가 가능한 객체만 취급 가능. 예를 들어, 래퍼 클래스, String 클래스
		//사용자 정의 클래스는 크기 비교를 위해서 compareTo() 메소드에 대한 오버라이딩 필수
